...
from api.assistant import routes as assistant_routes
...
async def lifespan(_: FastAPI):
    log.info("starting SugboDoc API (env=%s, db=%s, rag=%s)",
             settings.ENV, "postgres" if db.enabled() else "memory", config.USE_RAG)
    if db.enabled():
        db.init_db()
...
app = FastAPI(title="SugboDoc API", lifespan=lifespan)
...
def readyz() -> JSONResponse:
    ...
    body = {
        ...
        "active_sessions": assistant_routes.session_count() if db_state != "down" else None,
    }
